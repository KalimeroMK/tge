...
if (!defined('_FF_FTR_MODE')) {
    define('_FF_FTR_MODE', 'full');
}
...
if (file_exists('custom_init.php')) {
    require 'custom_init.php';
}
...
if (!empty($options->proxy_servers)) {
    if (isset($_REQUEST['proxy'])) {
        ...
        if (!$options->allow_proxy_override) {
            die('Proxy overriding is disabled.');
        }
...
if ($options->caching) {
    ...
    if ($check_cache) {
        ...
        if ($data = $cache->load($cache_id)) {
            ...
            if (headers_sent()) {
                die('Some data has already been output, can\'t send RSS file');
            }
            if ($callback) {
                echo "$callback($data);";
            } else {
...
foreach ($items as $key => $item) {
    ...
    if (isset($mime_info['mime'])) {
        $newitem->addElement('dc:format', $mime_info['mime']);
    }
    ...
    if (isset($effective_url)) {
        ...
        if ($favour_effective_url) {
            $newitem->setLink(remove_url_cruft(utf8_encode($effective_url)));
        }
    } else {
        ...
        if ($enclosures = $item->get_enclosures()) {
            foreach ($enclosures as $enclosure) {
                ...
                if ($enclosure->get_length()) {
                    $enc['fileSize'] = $enclosure->get_length();
                }
                if ($enclosure->get_type()) {
                    $enc['type'] = $enclosure->get_type();
                }
                if ($enclosure->get_medium()) {
                    $enc['medium'] = $enclosure->get_medium();
                }
                if ($enclosure->get_expression()) {
                    $enc['expression'] = $enclosure->get_expression();
                }
                if ($enclosure->get_bitrate()) {
                    $enc['bitrate'] = $enclosure->get_bitrate();
                }
                if ($enclosure->get_framerate()) {
                    $enc['framerate'] = $enclosure->get_framerate();
                }
                if ($enclosure->get_sampling_rate()) {
                    $enc['samplingrate'] = $enclosure->get_sampling_rate();
                }
                if ($enclosure->get_channels()) {
                    $enc['channels'] = $enclosure->get_channels();
                }
                if ($enclosure->get_duration()) {
                    $enc['duration'] = $enclosure->get_duration();
                }
                if ($enclosure->get_height()) {
                    $enc['height'] = $enclosure->get_height();
                }
                if ($enclosure->get_width()) {
...
function get_self_url()
{
    ...
    if (isset($_GET['html'])) {
        $self .= '&html='.urlencode($_GET['html']);
    }
    if (isset($_GET['accept'])) {
        $self .= '&accept='.urlencode($_GET['accept']);
    }
    if (isset($_GET['max'])) {
        $self .= '&max='.(int)$_GET['max'];
    }
    if (isset($_GET['links'])) {
        $self .= '&links='.urlencode($_GET['links']);
    }
    if (isset($_GET['images'])) {
        $self .= '&images='.urlencode($_GET['images']);
    }
    if (isset($_GET['exc'])) {
        $self .= '&exc='.urlencode($_GET['exc']);
    }
    if (isset($_GET['format'])) {
        $self .= '&format='.urlencode($_GET['format']);
    }
    if (isset($_GET['callback'])) {
        $self .= '&callback='.urlencode($_GET['callback']);
    }
    if (isset($_GET['l'])) {
        $self .= '&l='.urlencode($_GET['l']);
    }
    if (isset($_GET['lang'])) {
    ...
    if (isset($_GET['use_extracted_title'])) {
        $self .= '&use_extracted_title';
    }
    if (isset($_GET['use_effective_url'])) {
        $self .= '&use_effective_url';
    }
    if (isset($_GET['content'])) {
        $self .= '&content='.urlencode($_GET['content']);
    }
    if (isset($_GET['summary'])) {
        $self .= '&summary='.urlencode($_GET['summary']);
    }
    if (isset($_GET['debug'])) {
        $self .= '&debug';
    }
    if (isset($_GET['parser'])) {
        $self .= '&parser='.urlencode($_GET['parser']);
    }
    if (isset($_GET['proxy'])) {
        $self .= '&proxy='.urlencode($_GET['proxy']);
    }
    if (isset($_GET['siteconfig'])) {
        $self .= '&siteconfig='.urlencode($_GET['siteconfig']);
    }
    return $self;
}
