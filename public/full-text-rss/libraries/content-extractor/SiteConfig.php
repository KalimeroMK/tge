...
class SiteConfig
{
    ...
    {
        if ($use_default) {
            return (isset($this->insert_detected_image)) ? $this->insert_detected_image : $this->default_insert_detected_image;
        }
    ...
    public static function build($host, $exact_host_match=false)
    {
        ...
        if (!$host || (strlen($host) > 200) || !preg_match(self::HOSTNAME_REGEX, ltrim($host, '.'))) {
            return false;
        }
        // got a merged one?
